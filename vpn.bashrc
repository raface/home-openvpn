...
case "$1" in
  media)
    if docker ps | grep -v grep | grep "vpn-media" > /dev/null 2>&1; then
      ...
      docker run -d --name vpn-media --rm -i -t --privileged --sysctl net.ipv6.conf.all.disable_ipv6=0 -p 127.0.0.1:4446:22 -e "SSH_KEY=$(cat ~/.ssh/id_rsa.pub)" openvpn
      sleep 3
    fi
  ...
  media1)
    if docker ps | grep -v grep | grep "vpn-media1" > /dev/null 2>&1; then
      ...
      printf "Starting VPN Docker \n";
      docker build --force-rm -t openvpn /home/rafael/vpn/openvpn/
      docker run -d --name vpn-media1 --rm -i -t --privileged --sysctl net.ipv6.conf.all.disable_ipv6=0 -p 127.0.0.1:4444:22 -e "SSH_KEY=$(cat ~/.ssh/id_rsa.pub)" openvpn
      sleep 3
    fi
  ...
  web)
    if docker ps | grep -v grep | grep "openvpn" > /dev/null 2>&1; then
      ...
      printf "Starting VPN Docker \n";
      docker build --force-rm -t openvpn /home/rafael/vpn/openvpn/
      docker run -d --name vpnweb --rm -i -t --privileged --sysctl net.ipv6.conf.all.disable_ipv6=0 -p 127.0.0.1:4448:22 -e "SSH_KEY=$(cat ~/.ssh/id_rsa.pub)" openvpn
      sleep 3
    fi
